...
Meeting Mirror is a personal meeting coach that analyzes the selected user’s speaking behavior, maps other participants’ explicit needs and evidence-backed possible goals, and creates concrete follow-up actions.
...
1. Open the public page without an account.
2. Choose one of five Korean sample meetings or paste a labeled transcript.
...
- Include five polished 3–5 speaker Korean samples: sprint planning, design review, client requirements, student research, and incident review.
...
| Completeness · 16% | Golden path, five samples, custom transcript validation, SSE progress, errors, response contracts, timeout, unit/API/public smoke tests. |
| UX/workflow · 12% | Preselected sample/self, consent → analyze flow, semantic labels/IDs, responsive layout, loading/error/status announcements, copy/download/reset. |
...
- The public credential-free deployment uses the deterministic demo unless BYOK secrets are configured; the provenance badge always identifies this.
...
Audio ingestion, diarization, live meeting participation, emotion/sentiment detection, employee scoring, user history, organization analytics, calendar sync, and sending follow-up messages.
